...
import { onDeleteTodoList } from "../../actions/todoListsActions";
import { TodoLists, TodoListItem, Icon, Title } from "./TodoListStyle";

export const TodoList = ({ onClickList, icon, title, id }) => {
  const dispatch = useDispatch();
  const activeList = () => {
    onClickList(id);
  };
  const removeList = () => {
    dispatch(onDeleteTodoList(id));
  };
  return (
    <TodoLists>
      <TodoListItem onClick={activeList}>
        {icon ? (
          <Icon color={icon.hex} />
        ) : (
          <img src="/img/allTask.svg" alt="All task" />
        )}
        <Title>{title}</Title>
        {icon ? (
          <svg
